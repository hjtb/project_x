...
def test_sql_insert_params():
    ...
    sql = """
        insert into users 
        (email, password, email_confirmed, date_created) 
        VALUES 
        (:email, :password, :email_confirmed, :date_created)
    """
    ...
    query_parameters = [
        dict(
            email=f"delete_this_{secrets.token_urlsafe(7)}@gmail.com",
    ...
    for query_parameter in query_parameters:
        ...
        try:
            ...
            roles = db.session.execute("select * from roles")
            for role in roles:
                print(f"The user id is {role.id}, the role is: {role.role_name}")
            you_can_break_here = True
